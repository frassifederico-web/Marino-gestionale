...
async function printTodayReservationsSheet(){
  ...
  const n=rows.length;
  /* Caratteri volutamente grandi. Solo con liste molto lunghe si riducono quanto basta per restare su un A4. */
...
'''
marker='function _renderMapBase';if(marker not in s)raise SystemExit('_renderMapBase non trovata per stampa prenotazioni');s=s.replace(marker,patch+'\n'+marker,1)
m=re.search(r'Object\.assign\(window,\{([^}]*)\}\);',s);if not m:raise SystemExit('Object.assign(window,...) non trovato');items=m.group(1)
...
if '</head>' not in s:raise SystemExit('head non trovato');s=s.replace('</head>',css+'</head>',1)
if 'size:210mm 297mm' not in s or 'width:194mm' not in s or 'font-size:30px' not in s or 'Array.from({length:5})' not in s:raise SystemExit('Formato A4 verticale grande non inserito')
p.write_text(s)
