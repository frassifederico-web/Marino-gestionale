...
# Regole sala interna 10/09/2026 v4.
# Singoli piccoli: 1-3 pax, forzabili a 4.
# Una prenotazione da 1/2/3 pax usa SEMPRE un solo tavolo, mai un accorpamento.
# Accorpamenti: 1-2-3 ; 11-12-13-14-15-16-21-22 ; 21-22-23-24-25 ; 5-Jolly solo 4 pax.
...
function marinoFiveJollyPair(codes){
  ...
  const names=codes.map(c=>String(allTables.find(t=>t.code===c)?.name||allTables.find(t=>t.code===c)?.label||c).trim().toLowerCase());
...
# Forzatura 4 su singolo piccolo.
...
new_bulk_range=r'''function bulkAreaRange(area,codes){
  ...
  if(n===1){const mn=Number(ts[0].single_min_covers||1),mx=Number(ts[0].single_max_covers||1);return [mn,mx,marinoSmallInternalCode(ts[0].code)?4:mx]}
...
new_candidates=r'''function bulkAreaInternalCandidates(r){
  ...
  const add=codes=>{
    ...
    if(party<=3&&codes.length>1)return;
    ...
    const allowedMax=(codes.length===1&&marinoSmallInternalCode(codes[0])&&party===4)?hard:(r.forced?hard:mx);
    if(party<mn||party>allowedMax)return;
    seen.add(key);out.push({codes,mn,mx,hard,waste:allowedMax-party});
  };
  ...
  const five=tables.find(t=>{const n=String(t.name||t.label||t.code).trim().toLowerCase();return n==='5'||n.endsWith(' 5')||n.includes('tavolo 5')});
  const jolly=tables.find(t=>String(t.name||t.label||t.code).toLowerCase().includes('jolly'));
...
s=s.replace('</head>',legacy+'<style id="marino-internal-tables-sep10-v4"></style></head>',1)
for required in ['marinoInternalComboValid','marinoSmallInternalCode','marinoFiveJollyPair','marino-internal-tables-sep10-v4','Per 1, 2 o 3 coperti deve essere usato un solo tavolo']:
