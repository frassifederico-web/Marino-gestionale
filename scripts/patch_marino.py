...
# Prenotazioni: ordine cronologico + precedente/successivo utilizzo degli stessi tavoli.
...
# Tavoli: 1h45 standard, 1h30 forzabile; capacità visibile e ordinamento per adeguatezza.
...
function selectionRange(){
  ...
  if(ts.length===1)return {mn:Number(ts[0].single_min_covers||1),mx:Number(ts[0].single_max_covers||1)};
  let mn=ts.reduce((a,t)=>a+Math.min(Number(t.single_min_covers||1),3),0);
  let mx=ts.reduce((a,t)=>a+Math.min(Number(t.single_max_covers||1),3),0);
  return {mn,mx};
}
function renderSelectionSummary(){
  let el=$('tableSelectionSummary');if(!el)return;
  let r=selectionRange(),party=Number($('party').value||0);
  if(!r){el.innerHTML='<b>'+party+' coperti richiesti</b><div class="muted">Scegli prima i tavoli indicati come IDEALE.</div>';return}
...
new='<section id="bookings" class="page"><div class="card"><div class="actions bookingHead"><h3>Prenotazioni</h3><button class="primary" onclick="openBooking()">+ Nuova prenotazione</button></div><div class="muted" style="margin:5px 0 10px">Ordine cronologico · rimpiazzi precedente e successivo indicati sotto la prenotazione.</div><div id="bookingList"></div></div></section>'
if old not in s: raise SystemExit('Sezione Prenotazioni non trovata')
s=s.replace(old,new,1)

# Menu statico per gli admin; modifica riservata al principal nell'area Accessi.
...
s,n=re.subn(r"function renderMenu\(\)\{.*?\}\s*async function addMenu",new_render_menu+'\nasync function addMenu',s,count=1,flags=re.S)
...
# Supabase ESM: compatibile con Safari iOS e Chrome Android.
...
# Forzatura temporale backend: mostra la conferma quando il backend richiede forzatura.
...
# Layout responsive condiviso iOS/Android.
css=r'''
...
@media(max-width:720px){
 body{font-size:15px}.wrap{padding:9px 9px calc(94px + env(safe-area-inset-bottom))}.card{padding:12px;border-radius:16px;margin-bottom:10px}header{min-height:68px;padding:9px 10px}header h1{font-size:20px}
 ...
 .modal .box>button.primary[onclick*="saveBooking"]{position:sticky;bottom:max(10px,env(safe-area-inset-bottom));z-index:6;width:100%;min-height:52px;font-size:17px;box-shadow:0 8px 20px rgba(6,63,120,.18)}
}
@media(max-width:390px){.grid4{grid-template-columns:1fr 1fr}.wrap{padding-left:7px;padding-right:7px}.dishHead{gap:7px}.tabs button{font-size:9.5px}}
</style>
'''
