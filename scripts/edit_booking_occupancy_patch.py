...
function renderEditOccupancyLegend(){
  ...
  box.style.display='block';
  box.innerHTML='<div class="editOccTitle">Tavoli occupati nel servizio</div><div class="editOccGrid">'+occupied.map(x=>'<div class="editOccRow"><b>'+esc(x.t.label||x.t.code)+'</b><span>'+esc(x.r.guest_name||'—')+'</span><span>'+Number(x.r.party_size||0)+' cop.</span><span>'+esc(hhmm(x.r.arrival_time))+'</span></div>').join('')+'</div>';
}
  ...
  if(editing){
    ...
    allTables.filter(t=>t.area===room&&t.active).forEach(t=>{
      ...
      if(rs.length){
        ...
        d.innerHTML=rs.map(r=>'<div><b>'+esc(r.guest_name||'—')+'</b> · '+Number(r.party_size||0)+' cop. · '+esc(hhmm(r.arrival_time))+'</div>').join('');
        card.appendChild(d);
      }
    });
  }
...
.editOccRow{display:grid;grid-template-columns:minmax(70px,1fr) minmax(100px,2fr) auto auto;gap:7px;align-items:center;padding:6px 7px;border-radius:8px;background:#fff;border:1px solid #e5edf3;font-size:11px;color:#102c45}
...
@media(max-width:720px){.editOccRow{grid-template-columns:1fr 1.6fr auto;font-size:10px}.editOccRow span:last-child{grid-column:1/-1;color:#526574}.editOccupancyLegend{padding:8px}}
</style>'''
if '</head>' not in s:
    raise SystemExit('head non trovato per dettaglio occupazione modifica')
s=s.replace('</head>',css+'</head>',1)

if 'editOccupancyLegend' not in s or 'editOccInline' not in s or 'Tavoli occupati nel servizio' not in s:
    raise SystemExit('Dettaglio occupazione modifica non inserito')
