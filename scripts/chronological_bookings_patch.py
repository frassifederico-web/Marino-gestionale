...
new_render=r'''async function renderBookings(){
  ...
  host.innerHTML=days.map(iso=>{
    ...
    let cards=rows.map(r=>{
      ...
      return '<div class="chronoBookingRow"><div class="chronoBookingMain"><b><span class="reservationNo">#'+reservationDisplayNo(r)+'</span> · '+hhmm(r.arrival_time)+' · '+esc(r.guest_name)+' · '+Number(r.party_size||0)+' coperti</b> '+state+'<div class="muted">'+serviceLabel(r.service_code)+' · '+esc(labelsFor(r.id)||'Tavolo da assegnare')+' · '+esc(r.area||'')+'</div></div><div class="chronoQuickActions"><div class="chronoActionLine chronoArrivalLine">'+arriveAction+'</div><div class="chronoActionLine chronoOpenLine"><button class="secondary chronoOpenBtn" title="Apri prenotazione" aria-label="Apri prenotazione" data-open-chrono="'+esc(r.id)+'" data-date="'+esc(r.service_date)+'" data-service="'+esc(r.service_code)+'"><span class="chronoActionIcon">›</span><span class="chronoActionText">Apri</span></button></div></div></div>';
    }).join('');
...
.chronoBookingMain{min-width:0;flex:1}.chronoBookingMain>b{font-size:13px;line-height:1.25}.chronoBookingMain .muted{font-size:11px;line-height:1.25;margin-top:2px}.chronoQuickActions{display:flex;flex-direction:column;align-items:stretch;justify-content:center;gap:3px;flex:0 0 84px}.chronoActionLine{display:flex;width:100%}.chronoActionLine button{width:100%;display:flex;align-items:center;justify-content:center;gap:4px;min-height:27px;padding:3px 5px;font-size:9px;white-space:nowrap;border-radius:7px;font-weight:850}.chronoActionIcon{font-size:11px;line-height:1}.chronoActionText{font-size:9px;line-height:1}.chronoArrived{background:#eef2f5!important;color:#34404a!important;border-color:#aeb6bd!important}.presenceBadge{margin-left:3px!important;font-size:7px!important;padding:2px 4px!important;letter-spacing:.02em!important}
@media(max-width:720px){.chronoDay{margin-bottom:8px}.chronoDayHead{padding:8px 9px}.chronoDayHead b{font-size:14px}.chronoDayHead span{font-size:10px}.chronoDayList{padding:0 8px}.chronoBookingRow{align-items:center;padding:6px 0;gap:6px}.chronoBookingMain>b{font-size:12px}.chronoBookingMain .muted{font-size:10px}.chronoQuickActions{flex:0 0 76px;gap:3px}.chronoActionLine button{min-height:25px!important;padding:3px 4px!important;border-radius:6px!important}.chronoActionIcon{font-size:10px}.chronoActionText{font-size:8.5px}.presenceBadge{font-size:6.8px!important;padding:2px 3px!important}}
@media(max-width:390px){.chronoDayList{padding:0 6px}.chronoBookingRow{gap:5px}.chronoBookingMain>b{font-size:11.5px}.chronoBookingMain .muted{font-size:9.5px}.chronoQuickActions{flex-basis:70px}.chronoActionLine button{min-height:24px!important;padding:2px 3px!important}.chronoActionIcon{font-size:9px}.chronoActionText{font-size:8px}}
</style>'''
